...
from app.generated.messages_pb2 import (ErrorTaskResponse, MessageReportTask,
                                        ReportTaskResponse, WrapperResponse)
    ...
    try:
        ...
        if is_semi:
            callback_data = WrapperResponse(
                task_id=task_id, semi_report=ReportTaskResponse(text=response)
            )
        else:
            callback_data = WrapperResponse(
                task_id=task_id, report=ReportTaskResponse(text=response)
            )
